...
def read(scan, param, files, ycut):
    ...
    graph = plot.TGraphFromTree(limit, param, '2*deltaNLL', 'quantileExpected > -1.5 && 2*deltaNLL >= 0')
    graph.SetName(scan)
    graph.Sort()
...
parser.add_argument('--Not_showPoints', action='store_false', help='show the 1 sigma label')
...
else:main_scan['graph'].SetLineStyle(2)
main_scan['graph'].SetLineWidth(2)
...
save_graph.GetXaxis().SetLabelSize(0.01)
