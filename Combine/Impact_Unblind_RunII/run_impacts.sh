...
if [[ $STEP == "t2w" ]]; then
	...
	python combineTool.py -M Impacts -d Datacard_${YEAR}.root -n _scan_fa3_ggH_from_AC_HGG_HPlus2Jets -m 125.38  --freezeParameters MH --setParameterRanges muV=0,5:mu_ggH=0.5,1.5:fa3_ggH=-1,1:fa3=-1:1 --setParameters muV=1,mu_ggH=1,fa3_ggH=0,fa3=0 --X-rtd MINIMIZER_analytic --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 1 -v 1 --doInitialFit --robustFit 1 --job-mode condor --sub-opts='+JobFlavour="tomorrow"' --task-name doInitialFit 

elif [[ $STEP == "impacts-scans" ]]; then

	python combineTool.py -M Impacts -d Datacard_${YEAR}.root -n _scan_fa3_ggH_from_AC_HGG_HPlus2Jets -m 125.38  --freezeParameters MH --setParameters muV=1,mu_ggH=1,fa3_ggH=0,fa3=0 --setParameterRanges muV=0,5:mu_ggH=0.5,1.5:fa3_ggH=-1,1:fa3=-1:1 --X-rtd MINIMIZER_analytic --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 1 -v 1 --doFits --robustFit 1 --job-mode condor --sub-opts='+JobFlavour="tomorrow"' --task-name doFits  
