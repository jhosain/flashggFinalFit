...
def get_options():
  ...
  parser.add_option("--year", dest="year", default='2018', help="Year to include in plot")
...
# Make plot
colorMap = {'muV':ROOT.kAzure-9,'mu_ggH':ROOT.kOrange-4,'fa3_ggH':ROOT.kGreen-6,'fa3':ROOT.kMagenta-9}
...
for pidx in range(nPOIs):
  ...
  #graphs[poi].SetFillColor(ROOT.kGreen)
  binidx = 1
  for systVals in [ExpSystVals,TheorySystVals,OtherSystVals]:
    for gr,vals in systVals.iteritems():
...
lat2.DrawLatex(0.9,0.92,"138 fb^{-1} (13 TeV)")

extStr = "_relative" if opt.doRelative else ""
canv.Print("syst_%s_%s.png" % (opt.pois, opt.year))
canv.Print("syst_%s_%s.pdf" % (opt.pois, opt.year))
