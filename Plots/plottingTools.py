...
def makeSplusBPlot(workspace,hD,hSB,hB,hS,hDr,hBr,hSr,cat,options,dB=None,reduceRange=None):
  ...
  if options.unblind:
    ...
    hSB['pdfNBins'].SetLineColor(ROOT.kRed)
    hSB['pdfNBins'].SetLineStyle(1)
  ...
  #lat0.DrawLatex(0.55,0.92,"138 fb^{-1} (13 TeV)")
  ...
  if(options.loadSnapshot is not None):
    ...
    lat0.DrawLatex(0.15,0.77,"#scale[0.6]{#vec{#alpha} = (#mu_{f}, #mu_{V}, f_{a3}^{ggH})}")
