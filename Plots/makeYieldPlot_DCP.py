...
g1_signal_bsmmix_weighted = sum(signal_bsmmix_weighted[i] for i in group1_indices)
...
g2_signal_bsmmix_weighted = sum(signal_bsmmix_weighted[i] for i in group2_indices)
...
merged_signal_bsmmix_weighted = [g1_signal_bsmmix_weighted, g2_signal_bsmmix_weighted]
...
# Plot SM (f_{a3} = 0)
ax.bar(range(len(merged_signal_weighted)), merged_signal_weighted, width=1, linewidth=1.0,
       align='center', label=r"$f_{a3}^{ggH}=0$,$f_{a3}=0$")


# Plot BSM MIX(f_{a3} = 0.5) as an overlaid orange bar with transparency and thick outline                                                 
#ax.bar(range(len(merged_signal_bsmmix_weighted)), merged_signal_bsmmix_weighted, width=1, linewidth=1.0,
       #align='center', alpha=0.2, edgecolor='red', color='red')
...
ax.bar(range(len(merged_signal_bsm_weighted)), merged_signal_bsm_weighted, width=1, linewidth=3.0,
       align='center', label=r"$f_{a3}^{ggH}=1.0$,$f_{a3}=0$", edgecolor='orange', color='orange', facecolor='none')
...
ax_ratio.set_xlabel("$D_{CP}^{ggH}$",fontsize=18)
...
plt.savefig('plots_stage03/fa3_%s_D_CP.png' % opt.out)
plt.show()
