...
def get_options():
    ...
    parser.add_option("--inputSeffJsonSM", dest="inputSeffJsonSM", 
                      default='jsons_test/catsSeff_sospbfa3_ggH_CMS_hgg_mass.json', help="")
    parser.add_option("--inputSeffBSMJsonSM", dest="inputSeffBSMJsonSM", 
                      default='jsons_test/catsSeff_sospbfa3_ggH_bsm_CMS_hgg_mass.json', help="")
    parser.add_option("--inputBeffJsonSM", dest="inputBeffJsonSM", 
                      default='jsons_test/catsBeff_sospbfa3_ggH_CMS_hgg_mass.json', help="")
    parser.add_option("--inputSeffJsonBSM", dest="inputSeffJsonBSM", 
                      default='jsons_test/catsSeff_sospbfa3_ggH_bsm_CMS_hgg_mass.json', help="Group of cats")
    parser.add_option("--inputJsondataratio", dest="inputJsondataratio", 
                      default='jsons_test/catsDataRatioWeighteff_sospbfa3_ggH_CMS_hgg_mass.json', help="Extension for saving")
    parser.add_option("--inputJsonDataErrRatio", dest="inputJsonDataErrRatio", 
                      default='jsons_test/catsDataRatioWeightErr_sospbfa3_ggH_CMS_hgg_mass.json', help="Extension for saving")
    parser.add_option("--inputJsondata", dest="inputJsondata", 
                      default='jsons_test/catsDataeff_sospbfa3_ggH_CMS_hgg_mass.json', help="Extension for saving")
    parser.add_option("--inputJsonweight", dest="inputJsonweight", 
                      default='jsons_test/catsWeights_sospbfa3_ggH_CMS_hgg_mass.json', help="Extension for saving")
    parser.add_option("--inputJsonbsmweight", dest="inputJsonbsmweight", 
                      default='jsons_test/catsWeights_sospbfa3_ggH_bsm_CMS_hgg_mass.json', help="Extension for saving")
...
ratio_sm = np.array(merged_data_points) / np.array(merged_signal_weighted, dtype=float)
ratio_bsm = np.array(merged_data_points) / np.array(merged_signal_bsm_weighted, dtype=float)
ratio_sm_err = np.array(merged_data_err) / np.array(merged_signal_weighted, dtype=float)
ratio_bsm_err = np.array(merged_data_err) / np.array(merged_signal_bsm_weighted, dtype=float)

ax_ratio.scatter(range(len(ratio_sm)), ratio_sm, color='#5790fc', zorder=3)
ax_ratio.scatter(range(len(ratio_bsm)), ratio_bsm, color='#f89c20', zorder=3)
ax_ratio.errorbar(range(len(ratio_sm)), ratio_sm, yerr=ratio_sm_err, fmt='none',
                  c='#5790fc', capsize=5, zorder=2, xerr=0.5)
ax_ratio.errorbar(range(len(ratio_bsm)), ratio_bsm, yerr=ratio_bsm_err, fmt='none',
                  c='#f89c20', capsize=5, zorder=2, xerr=0.5)

ax_ratio.axhline(1, color="black", linestyle="--", linewidth=1.5)
ax_ratio.set_ylabel("Data/MC")
ax_ratio.set_xlabel("$D_{0^{-}}^{ggH}$")
...
plt.savefig('plots/fa3_%s_D0_minus_fa3.png' % opt.out)
plt.show()
