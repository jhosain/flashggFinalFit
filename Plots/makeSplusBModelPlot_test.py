...
if opt.inputWSFile is not None:
  ...
  if opt.doBSM:
    # Set the fa3_ggH parameter to 1 for the BSM scenario
    var_ggH = w.var("fa3_ggH")
    if var_ggH:
      var_ggH.setVal(1.)
    else:
      print "WARNING: fa3_ggH not found in workspace."
      
    # Also set the fa3 parameter to 1 for BSM
    var_fa3 = w.var("fa3")
    if var_fa3:
      var_fa3.setVal(1.)
    else:
      print "WARNING: fa3 not found in workspace."
...
if opt.loadWeights != '':
  ...
  if opt.doCatWeights:
    ...
    if opt.saveWeights:
      print "      * Saving S/S+B weights to json file: ./jsons_test/catsWeights_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0])
      if not os.path.isdir("./jsons_test"): os.system("mkdir ./jsons_test")
      with open("./jsons_test/catsWeights_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catsWeights,jsonfile)
    if opt.doYield:
      print "      * Saving Seff,Beff  to json file: ./jsons_test/catsSeff_sospb%s_%s.json and ./jsons_test/catsBeff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0],opt.ext,opt.xvar.split(",")[0])
      if not os.path.isdir("./jsons_test"): os.system("mkdir ./jsons_test")
      with open("./jsons_test/catsBeff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catsBeff,jsonfile)
      with open("./jsons_test/catsSeff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catsSeff,jsonfile)
...
for cidx in range(len(cats)):
  ...
  if opt.doYield:
    ...
    print "      * Saving yield weighted data to json file: ./jsons_test/catsDataEff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0])
    """
    if not os.path.isdir("./jsons_test"): os.system("mkdir ./jsons_test")
    with open("./jsons_test/catsDataRatioeff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catsDataRatioEff,jsonfile)
    ...
    with open("./jsons_test/catsDataRatioErr_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catErrData,jsonfile)
    """
    
  
    print "      * Saving yield weighted data to json file: ./jsons_test/catsDataEff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0])
    if not os.path.isdir("./jsons_test"): os.system("mkdir ./jsons_test")
    with open("./jsons_test/catsDataeff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catsDataEff,jsonfile)
  ...
  if opt.doYield:
    ...
    print "      * Saving yield weighted data to json file: ./jsons_test/catsdweighteff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0])
    if not os.path.isdir("./jsons_test"): os.system("mkdir ./jsons_test")
    with open("./jsons_test/catsDataRatioWeighteff_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catsDataWeightRatio,jsonfile)
    print('-----------')
    err_tot = 0
    for Bin in range(Bin_from, Bin_to+1):
    ...
    with open("./jsons_test/catsDataRatioWeightErr_sospb%s_%s.json"%(opt.ext,opt.xvar.split(",")[0]),'w') as jsonfile: json.dump(catErrDataWeight,jsonfile)
