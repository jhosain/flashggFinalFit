...
cp TreeMakerForWorkspace_multiclassBDT_DCP_3D.C $InputDirectory
...
for ((i = 0; i < $Nbinx; i++)); do
  ...
  for ((j = 0; j < $Nbiny; j++)); do
    ...
    root -l -b -q ../TreeMakerForWorkspace_multiclassBDT_DCP_3D.C\($i,$j,$k,$bin_index\) &> run$bin_index.log &  # run in the background bin by bin
